											...
											<div class="col-md-6">
												<div class="form-group">
													<label for="txt_orderlinesname" class="small">Order
														Lines Name</label> <input type="text" name="OrderLinesName"
														class="form-control" id="order_lines_name_input"
														placeholder="Enter OrderLinesName"
														value="${orderLines.orderLinesName}" />
												</div>
											</div>
													...
													<label for="sel_order" class="small">Order</label> <select
														...
														<c:forEach items="${all_order}" var="item">
															<option value="${item.orderId}"
																${fn:contains(orderLines.order, item) ? 'selected' : ''}>${item.getNaturalKey()}</option>
														</c:forEach>
													</select>
												</div>
											</div>
